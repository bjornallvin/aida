...
function getDeviceName(device: Device): string {
  return device.attributes.customName || 
...
export function findBestDeviceMatch(
  ...
  for (const device of candidateDevices) {
    ...
    if (opts.enablePartialMatch) {
      ...
      const deviceWords = normalizedDeviceName.split(' ').filter(w => w.length > 2);
      
      let partialScore = 0;
      for (const inputWord of inputWords) {
        for (const deviceWord of deviceWords) {
          if (inputWord.includes(deviceWord) || deviceWord.includes(inputWord)) {
            partialScore += 0.5;
          }
        }
      }
      
      const partialConfidence = Math.min(0.8, partialScore / Math.max(inputWords.length, deviceWords.length));
